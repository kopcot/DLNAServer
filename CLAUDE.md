...
>
> **Version `1.1.0926`.** Every change bumps the assembly version - the rule, and who may bump which part,
...
> plays from it, **1,065 tests at 0 warnings**, no vulnerable packages. **The NAS is several batches behind
...
**Two ports, one process.** One port serves UPnP/DLNA and media, the other the Blazor admin UI. `ServerOptions` defaults to `26851`/`26852`, but the shipped `config.json` sets `26852`/`26853` so this server can run alongside the reference implementation on 26851 — read the live values from `config.json`, not the defaults. Controllers are gated per request by path prefix through `AdminOrMediaPortEndpointFilter` (`/admin` routes to the admin port, everything else to the media port; `AdminMediaController` straddles both as the admin UI's own preview door onto the same content). `RequirePortEndpointFilter` guards only the `/health`, `/admin/health` and `/health/ready` endpoints, and Razor components are confined to the admin port by `AdminSurfaceMiddleware`. A renderer on the media port still gets a 404 for anything admin-only. The two ports must differ or startup fails.
...
- **An architecture change updates the picture in the same change.**
    - *Architecture change* means any of: a hosted service, controller, SOAP service, repository or store added, removed or renamed; a new caller of one of them; a dependency between projects; a new store of state.
    - Edit the Mermaid block under *How the pieces talk* in `docs/architecture.md` first, from the code rather than from the old picture: the gitdiagram original this replaced had three arrows the code never did.
    - Then re-render `docs/architecture.png` from that block, so the two never disagree. Render it with Mermaid 11 in a browser page: `theme: "base"`, `flowchart: { useMaxWidth: false, nodeSpacing: 25, rankSpacing: 60 }`, `themeVariables: { fontFamily: "Trebuchet MS, sans-serif", clusterBkg: "#f7f7f7", clusterBorder: "#dddddd", lineColor: "#333333", edgeLabelBackground: "#f3e8ff" }`, on a `#f3e8ff` page with a right-aligned `ZEN DLNA Server - architecture, <date>` footer. Take a full-page screenshot.
    - Keep the `classDef` colours: blue for protocol and delivery, pink for the admin port, amber for the library, green for state.
    - Point an edge from a background service to `DatabaseReadySignal` as "waits for". The other direction pulls the persistence group to the top of the layout.
