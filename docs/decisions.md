...
- **The Dashboard's counts re-read `Files` only when the library changed, and a new writer has to say so.** `CountByKindAsync` reads every row (70-150 ms on the NAS, 127 of the 139 slow-query entries after the 2026-09-23 deploy), and the 30-second refresh kept the disc from ever spinning down while a Dashboard sat open; each page load also counted twice, pre-rendered and interactive. `LibraryCountsCache` (Admin, singleton) now keeps one snapshot for every Dashboard, reused while `ILibraryChangeSignal.Generation` and the hidden-folder list are unchanged, with a 10-minute ceiling as the safety net. **Markers today:** every `LibraryIndexer.IndexAsync` pass, *Rebuild index* on Maintenance, and `FileTypeEditor`. Anything new that adds, removes, moves or re-types files outside those paths must call `MarkChanged`, or the Dashboard shows old figures for up to ten minutes.
- **A regenerated `config.json` carries the shipped file-type map, from `MediaFileExtensionDefaults` (Core).** It used to carry an empty map, which indexes nothing, so a server recovered from a lost file had no media even once its folders were set. The same factory backs *Restore the default file types* on Settings, so the two cannot drift. It is deliberately not a default on `LibraryOptions.MediaFileExtensions`, for the `ConfigurationBinder` append trap described under `ExcludeFolders` in `CLAUDE.md`. `SourceFolders` stays empty in the written file on purpose: the application-folder fallback happens at bind time in `DlnaOptionsDefaults`, which also adds the `Resources` and cache-folder exclusions - writing the folder into the file would skip those and index the server's own icons as photos.
- **The configuration system binds a JSON `null` as `""`.** So a `"ProfileName": null` in `config.json` arrives as an empty string, and `ProfileName ?? default` never fell back. `LibraryIndexer.BuildScanOptions` now treats blank as unset. The wire was never affected - `DlnaProtocolInfo.ResolveProfile` already treats blank as unset - but the stored `DlnaProfileName` was `""` instead of the type's profile. Any new reader of a nullable string option must test for blank, not null.
