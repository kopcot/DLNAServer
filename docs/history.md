...
build is at 0 warnings, and **1,065 tests pass** (483 unit, 560 integration, 22 architecture).
...
## 6u. Fourth whole-tree review and the asks around it (2026-09-26, `1.1.0926`)

A fourth `/review-all --full`, this time over the **whole tree** rather than a diff: 18 agents and 5 skills, the suite and a live drive of an isolated copy. Everything below was verified against the code before it was fixed; the report is kept out of the repository, as before. The maintainer asked for every valid finding to be fixed, and for four features alongside.

### Features asked for in the same batch

- **Subtitle filter with kinds.** `MediaFileSearchRequest.HasSubtitles` is a `SubtitlePresence?` - `Yes`, `Embedded`, `Linked`, `No` - rather than a `bool?`. `Embedded` and `Linked` ask whether the file has that kind, not only that kind: the maintainer's ruling was that a film with both may appear under either, but a file with only a linked subtitle must never appear under *embedded*.
- **Subtitle download on the preview page.** `GET /admin/media/subtitles/{id}` on `AdminMediaController` serves the one linked file as itself, or several as `<film>.subtitles.zip` with each entry under its path from the film's folder, so `film.en.srt` and `Subs/film.en.srt` stay apart. Only links `ISubtitleFileChecker` accepts are offered - the same rule the page counts with, so the button's wording (*Download subtitle* / *Download subtitles (zip)*) matches what arrives. Zipped in memory: subtitle files are tens of KB.
- **Dashboard.** `LibraryCountsDto.Subtitles` counts live links of visible files, reached through `Files` like `GetLanguagesAsync` so a hidden folder's links stay hidden. It is not part of `Total`. *Other* moved to its own row, still shown only when a file of no kind exists (the maintainer's ruling, after first asking for it always).
- **Subtitle types setting.** `Library.SubtitleFileExtensions` maps an extension to `Video` (a subtitle) or `Audio` (lyrics), with no default on the property - `DlnaOptionsDefaults` seeds `SubtitleFileExtensionDefaults` only when configuration names none, for the `ExcludeFolders` reason in `CLAUDE.md`. `SubtitleMatcher` and `SubtitlePath` take the map as a parameter, so Core stays free of options. The validator refuses an entry that is also a media type, in `MediaFileExtensions` or in the catalog the scanner falls back to. Settings has a *Subtitle types* panel with *Restore the default subtitle types*, which fills the form like its File types twin. **Removing a type** refuses its links at once in serving and in what televisions are offered (`DidlMapper.Offerable`), and the next scan deletes them, hand-added ones included - the same rule as a manual link into an excluded folder; a `Removed` marker stays.

### Findings fixed

- **A metadata timeout was saved as success.** `MediaProcessor` returned `MediaMetadataResult.Empty` on the ffprobe timeout, and only `null` counts as a failure, so the save wiped every stream row and stamped the file done. It returns `null` now. An internal constructor takes the probe as a delegate, because Xabe refuses to run without real binaries before it ever reads the token.
- **A wrong-typed `config.json` value escaped the last-good net.** Reproduced live before the fix: `"DebugMode": "yes"` gave `/health/ready` 500 and an `InvalidOperationException` from `ConfigurationBinder` in every reader. `LastGoodDlnaOptionsMonitor` now also serves last good for an `InvalidOperationException` whose `Source` is the binder's assembly, and nothing else. After the fix the same edit answers 503 "running on the last settings". It had no tests at all; it has four.
- **A share unmounting mid-reconcile could still empty the index.** `FindUnusableSourceFolders` ran once, before a reconcile that pages through every row; an empty mountpoint makes `IsDefinitelyAbsent` take its `DirectoryNotFoundException` arm, which answers "gone". The check now runs again per page that has missing rows, and the rest of the pass stops the moment it fires.
- **Uploads:** form fields are read at most 4 KB and a post carries at most 1000 parts (one huge field was an out-of-memory); the partial file is `<name>.<guid>.uploading`, so same-name uploads cannot delete each other's, and the move honours *keep existing*; a dropped client leaves no partial; the scan is requested before the device is remembered; a name with a control character is refused and every field in `uploadSecurity.log` is escaped; `ValidateUploadFolder` refuses a folder the library excludes. Because partials are now unique, a crashed upload's leftover is no longer overwritten by the next one of that name, so a successful upload removes partials of its own name idle for an hour.
- **Stop discarded a pending Recreate database.** Refused now - 409 on `/manage/stop`, a message on the page - while `IDatabaseResetSignal` is set. Keeping the restart flag instead would have turned Stop into a restart, the very thing `Reset()` exists to prevent.
- **The admin port answered the internet over IPv6.** `AdminSurfaceMiddleware` refuses admin-port requests from off the local network. The socket's own peer is recorded before `UseForwardedHeaders` rewrites it, so the Caddy path on loopback keeps working and a forged `X-Forwarded-For` does not; `X-Original-For` was not usable, since the forwarder leaves a client-sent one alone. A global IPv6 peer in the server's own /64 counts as local. The same `LocalNetworkAddress` helper now also filters SSDP (a reflector otherwise) and caps GENA subscriptions at 8 per device.
- **Large-object-heap churn:** the Dashboard built a sorted copy of every cache key every 5 s for a list it never showed (`ServedFileCache.ListPaths()` is separate now); `DidlSerializer` reserved 1,536 *chars* per object, a 98 KB buffer per full Browse page, on the false belief that a growing StringBuilder copies (it chains chunks). `BrowseResponse` serialises its empty Result once per process.
- **Persistence:** the scan-long context now forgets entities after `UpdateContentAsync` and `MoveOrRenameAsync`, as `InsertAsync` already did; the four delete-then-clear-stamp pairs run in transactions; `DbUpdateException` is unwrapped to its `DbException` where a caller catches the latter; `UploadDeviceRepository` takes the `TimeProvider`.
- **The watcher could loop full scans forever** once inotify ran out of watches. Restarts back off from a minute to 30, and one log line names `fs.inotify.max_user_watches`.
- **Smaller:** a retired metadata or thumbnail half is no longer re-probed when the other half is claimed; `FFmpegProvisioner` reads its flag through `Volatile`; a fatal database initialisation exits 1; `SettleAsync` skips its forced collection while an eviction's is running; `UpnpDeviceRegistry.Resolve` snapshots once; the SSDP listener no longer disposes a semaphore an in-flight reply still releases; `DatabaseReadyMiddleware` passes straight through once ready; thumbnails may hold at most 24 of the fill backlog's 32 slots, so a film asked for after a Browse page is still queued; the admin HEAD now agrees with its GET for a deleted-but-cached file; SVG responses carry `Content-Security-Policy: sandbox`; `/manage/directory/{id}` pages; the Dashboard and Library refresh loops survive a database error.
- **Shape, behaviour-neutral:** one thumbnail ladder (`IMediaContentResolver.ResolveThumbnailAsync`), one frame-thumbnail helper in `MediaProcessor`, `PathSegments.HasDotSegment`, one `LibraryIndexLock` registered as both the index lock and the admin gate, `AddDlnaUpnp`, `ApplyDlnaHeaders`, `GetFavicon` delegating, `PersistenceTestData`.
- **Docs and CI:** `CLAUDE.md` named the wrong port filter; `.env` is kept out of the build context; the remote overlay's process-wide `AllowedHosts` now takes `LAN_HOSTS`, or a television reaching the server by its local address got a 400; `release.yml` builds with read-only permissions and publishes in a separate job; a new architecture test checks the wire-contract docs, and caught `DidlResource.Url`.
- **A test that proved nothing:** `ClearAllThumbnailsAsync_RemovesTheRowAndTheStamp` looked the thumbnail up by the media file's id, so it passed whatever the code did.

### Not applied, and why

- **The single pruned scanner walk** was raised again by two reviewers; section 6i already declined it until it is measured. Still declined.
- **Tighter Kestrel header limits:** the defaults are already the bound, and a dev box's `localhost` cookie jar arriving at this port makes a tighter one a 431 risk.
- **Gating Library's *Recently added* refresh on the change generation:** processing does not mark changes, so a new preview would wait up to ten minutes instead of thirty seconds.
- **`FindByContentStampsAsync` through the `FullPath` index:** an interface change for a bounded cost.
- **The `…Abstractions 10.0.12` pins** came from an unreviewed Dependabot merge (PR #7) with no reason recorded; left for the maintainer.
- **The ffmpeg process possibly outliving its timeout** (Xabe cancels by writing `q`) needs a `ps` check on the NAS, not a code read; it joins the owed soak in 7b.
